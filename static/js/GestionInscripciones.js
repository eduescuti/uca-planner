...
function cambiarEstado(event, idForm) {
  try {
    ...
    // Obtén el formulario con el ID proporcionado
    var form = event.currentTarget;

    // Obtén el ID de la inscripción desde el atributo de datos
    var inscripcionId = form.dataset.inscripcionId;

    var idInscripcion = form.querySelector('[name="idInscripcion"]').value;
    ...
    // Obtén el valor del campo oculto 'idInscripcion'
    var idInscripcion = form.querySelector('[name="idInscripcion"]').value;

    // Realiza la llamada AJAX pasando el ID de la inscripción
    queryAjaxForm1('/cerrar_inscripcion', 'resCierre', form, idInscripcion);
  } catch (error) {
...
function queryAjaxForm1(url, idDest, idForm, method = "POST") {
  var formData = getDataForm1(idForm);                     // Obtener los pares 
  ...
  if (xhr) {
      ...
      xhr.onreadystatechange = function () {               // CARGA la función en el 'evento' del ajax onreadystatechange
          if (xhr.readyState == 4 && xhr.status == 200) {
              setDataIntoNode1(idDest, xhr.responseText)   // CARGAR la respuesta en el html destino
          }
      }
...
function getDataForm1(idForm) {
  var formData = new FormData();
  var form = document.getElementById(idForm);

  if (form) {
      var dataInputs = form.getElementsByTagName('input');
      var dataSelects = form.getElementsByTagName('select');

      if (dataInputs.length > 0 || dataSelects.length > 0) {
        data = document.forms[idForm].getElementsByTagName("input");                // Obtener los input
        for (let i = 0; i < data.length; i++) {                                        // recorrer los elementos del formulario
            if (data[i].name != undefined && data[i].value != undefined)
                if (data[i].type == 'text' || data[i].type == 'password') {
        ...
        return formData;       
          ...
          console.error('El formulario no contiene elementos input ni select.');
      }
  } else {
      console.error('Formulario no encontrado con ID:', idForm);
      return formData;
  }
}
...
function setDataIntoNode1(idDest, textHTML) {
  let oElement; // objeto
  let sNameTag; // string
